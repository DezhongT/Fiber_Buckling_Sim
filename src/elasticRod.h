...
class elasticRod
{
	...
	void setVertexBoundaryCondition(Vector3d position, int k, int p);
	void setThetaBoundaryCondition(double desiredTheta, int k, int p);
	void updateTimeStep();
	void computeElasticStiffness();
	void prepareForIteration();
	// void updateNewtonX(double *dx, double alpha);
	void updateNewtonX(VectorXd &DX, double alpha);
	...
	void modifymass(int i, double value);
	...
	void setVelocity(int k, Vector3d u);
	...
	void updateOpt(VectorXd &DX, double alpha);


	VectorXd xn;

	void useDataforSim(VectorXd m_x, VectorXd m_u, MatrixXd m_d1,

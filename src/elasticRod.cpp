...
void elasticRod::setVertexBoundaryCondition(Vector3d position, int k, int p)
{
	isConstrained[4*k] = p;
	isConstrained[4*k + 1] = p;
	isConstrained[4*k + 2] = p;
...
void elasticRod::modifymass(int i , double value)
{

	massArray[4*i] = value * massArray0[4*i];
	massArray[4*i+1] = value * massArray0[4*i+1];
	massArray[4*i+2] = value * massArray0[4*i+2];
}

void elasticRod::setVelocity(int k, Vector3d u1)
{
	for (int i = 0; i< 3; i++)
	{
		u(4*k+i) = u1(i);
...
void elasticRod::setThetaBoundaryCondition(double desiredTheta, int k, int p)
{
	isConstrained[4*k + 3] = p;
...
void elasticRod::updateNewtonX(VectorXd &DX, double alpha)
{
...
void elasticRod::updateOpt(VectorXd &DX, double alpha)
{
	for (int c=0; c < uncons; c++)
	{
		x[unconstrainedMap[c]] -= alpha * DX(c);
...
void elasticRod::storeX()
{
  xn = VectorXd::Zero(ndof);
	xn = x;

}


void elasticRod::recoverX()
{
	x = xn;
}


void elasticRod::useDataforSim(VectorXd m_x, VectorXd m_u, MatrixXd m_d1,
	MatrixXd m_d2, MatrixXd m_tangent, VectorXd m_refTwist)
{
	for (int i = 0; i < ndof; i++)
	{
		x(i) = m_x(i);
	...
	for (int i = 0; i<ne; i++)
	{
		...
		tangent(i, 0) = m_tangent(i, 0);
		tangent(i, 1) = m_tangent(i, 1);
		tangent(i, 2) = m_tangent(i, 2);

		refTwist(i) = m_refTwist(i);
	}
}
